...
static const int ARMR_GIFT_POS_BLOCK_COUNT = !fTestNet ? 184814 : 8100;
static const int FIRST_REDUCTION_POS_BLOCK_COUNT = !fTestNet ? 324022 : 8110;
static const int CHRISTMAS_GIFT_POS_BLOCK_COUNT = !fTestNet ? 433877 : 8120;
static const int FINAL_REDUCTION_POS_BLOCK_COUNT = !fTestNet ? 482989 : 8130;
static const int FLAT_POS_BLOCK_COUNT = !fTestNet ? 559241 : 8140;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, const CBlockIndex* pindex)
{
    ...
    else if (nHeight < FIRST_REDUCTION_POS_BLOCK_COUNT)
    {
        ...
        fTestNet ? LogPrintf("Calculating 15% reward\n") : LogPrintf("\n") ;
    }
    else if (nHeight < CHRISTMAS_GIFT_POS_BLOCK_COUNT)
    {
        nSubsidy = nRewardCoinYear * 5 * nCoinAge / 365;
        fTestNet ? LogPrintf("Calculating 5% reward\n") : LogPrintf("\n") ;
    }
    else if (nHeight < FINAL_REDUCTION_POS_BLOCK_COUNT)
    {
        nSubsidy = nRewardCoinYear * 10 * nCoinAge / 365;
        fTestNet ? LogPrintf("Calculating 10% reward\n") : LogPrintf("\n") ;
    }
    else if (nHeight < FLAT_POS_BLOCK_COUNT)
    {
        nSubsidy = 0.05 * nCoinAge / 365;
    }
    else
    {
