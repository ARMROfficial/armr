...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Armr " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strTestNetOnionSeed[][1] = {
		{"4ebm6fxpv7eclzc7.onion"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("Armr-dnsseed");
...
void ThreadOnionSeed2(void* parg)
{
    ...
    printf("%d addresses found from .ARMR seeds\n", found);
}
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Armr is probably already running."), addrBind.ToString().c_str());
        else
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_ONIONSEED] > 0) printf("ThreadARMRSeed still running\n");
