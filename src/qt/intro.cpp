...
{
    ui->setupUi(this);
    ui->storageLabel->setText(ui->storageLabel->text().arg(tr("ARMR")));
...
bool Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || GetBoolArg("-resetguisettings", false))
    {
        ...
        while(true)
        {
            ...
            try {
                ...
                QMessageBox::critical(0, tr("ARMR"),
     ...
     * override -datadir in the ARMR.conf file in the default data directory
     * (to be consistent with ARMRd behavior)
     */
    if(dataDir != getDefaultDataDirectory()) {
